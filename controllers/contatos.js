...
controller.criar = async function (req, res, next) {
    ...
    contact.create().then(model => {
        req.flash('messages', contact?.errors || 'Contato cadastrado com sucesso!');
        res.redirect('back');
    });
}
...
controller.editar = function (req, res, next) {
    ...
    contact.update().then(model => {
        req.flash('messages', `Contato editado!`);
        res.redirect('back');
    });
}

controller.apagar = function (req, res, next) {
    const contact = new Contato(req.body, req.file, req.session);

    contact.delete().then(model => {
        req.flash('messages', `Contato editado!`);
        res.redirect('back');
    });
}
